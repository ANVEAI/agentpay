...
describe("verifyPayment", () => {
  ...
  it("accepts a sufficient, recent, confirmed USDC transfer to payTo", async () => {
  ...
  it("rejects when not enough confirmations (reorg safety)", async () => {
    const req = createPaymentRequirement({ payTo: PAY_TO, amount: 1, resource: "/x" });
    receipts.set(TX, receipt(1_000_000n));
    latestBlock = 1n; // tx in block 1, latest 1 → 1 confirmation
    const r = await verifyPayment(req, { txHash: TX }, baseSepolia, { minConfirmations: 3 });
    expect(r.ok).toBe(false);
    expect(r.reason).toMatch(/confirmation/i);
  });

  it("rejects a malformed tx hash before hitting the chain", async () => {
    const req = createPaymentRequirement({ payTo: PAY_TO, amount: 1, resource: "/x" });
